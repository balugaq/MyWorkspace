...
TODO 24. （状态：待处理）
设置页面改版
改为更 modern 的软件页面（这个todo还没写完以后再写）

TODO 25. （状态：待处理）
重构页面，将其改为类似 Codex 的页面
具体为：
打开主界面后，默认直接打开 AI 对话页面（打开新对话或最新页面）
将所有分类内容（添加分类，内置模板，分类具体内容）全部合并为一个"随笔"工具，放在工具下。
sidebar不再需要，原有工具所占的区块直接剪切出sidebar，改为贴在页面左下角常驻显示即可。（可收起）
并定义工具上方原本sidebar剩余的位置为(左上侧)，左上侧是可收起的。

对于随笔工具，点击后打开随笔页面，并在(左上侧)改为显示原有的所有分类内容。（添加分类，内置模板，分类具体内容）

对于AI对话页面，改为在(左上侧)改为显示原有AI对话列表的内容。
