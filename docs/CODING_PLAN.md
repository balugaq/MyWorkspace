...
components/
  ...
  calendar-workspace.tsx     日历（月视图 + 当日详情，风格参考 .ref/SimpleCalendar）
...
| 日历 | 月视图（SimpleCalendar 风格）、日期导航、笔记（富文本+图片）、待办、事件、思维图截止任务、节日/生日标记（~渲染标记脚本~ 已弃用停用） |
| 全局搜索 | 范围筛选（全部/当前分类/日历/Todo/思维图）、高亮、跳转 |
| 设置 | 默认视图、主题、字号滑块、自定义快捷键（~~日历标记脚本~~ 已弃用停用）、配置文件源文本、图片缓存/暂存、含图备份 |
| 交互 | Ctrl+M 新建 / Ctrl+B 日历 / Ctrl+K 搜索、文本框 Ctrl+A 全选 / Ctrl+S 保存、底部状态栏 |
