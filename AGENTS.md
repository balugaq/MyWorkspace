...
| 入口 | `app/page.tsx`（主布局：侧边栏 + 工作区分发 + 状态栏 + 各弹窗 + `useGlobalShortcuts`；按 `view` 分发 `calendar`/`contacts`/`vault`/`ai-chat` 与分类工作区）；`app/layout.tsx`（系统字体栈 + `ThemeProvider` + `Toaster` + `VaultProvider`） |
| 状态 | `lib/store.ts` —— `useWorkspace`（Zustand + persist），含分类/章节/思维图节点与连线/日历/系统设置/贡献账本（`contributions`，Profile 热力图数据源）/AI 助手多会话（conversations + activeConversationId，各持完整消息上下文）/密码保险库会话（保险库仅存加密 blob，密钥不落盘）；流式请求由 `lib/ai/request-queue.ts` 全局队列持有——切换会话/切走视图都不中断在途请求，设置 `aiForceSync` 可把所有对话请求串行化（单队列） |
