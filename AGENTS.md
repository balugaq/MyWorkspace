...
| 状态 | `lib/store.ts` —— `useWorkspace`（Zustand + persist），含分类/章节/思维图节点与连线/日历/系统设置/AI 助手多会话（conversations + activeConversationId，各持完整消息上下文）/密码保险库会话（保险库仅存加密 blob，密钥不落盘；日历脚本字段已弃用停用） |
