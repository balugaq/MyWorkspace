...
| 状态 | `lib/store.ts` —— `useWorkspace`（Zustand + persist），含分类/章节/思维图节点与连线/日历/系统设置/AI 助手多会话（conversations + activeConversationId，各持完整消息上下文）/密码保险库会话（保险库仅存加密 blob，密钥不落盘）；流式请求由 `lib/ai/request-queue.ts` 全局队列持有——切换会话/切走视图都不中断在途请求，设置 `aiForceSync` 可把所有对话请求串行化（单队列） |
